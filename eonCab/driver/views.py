...
from eonCab.settings import idle_drivers, cancelled_ride
...
def add_vehicle(request):
    ...
    if driver.vehicle is None:
        jsn = request.data
        if not ('vehicle_number' in jsn and 'seat_capacity' in jsn and 'mileage' in jsn and 'type' in jsn):
            return Response(
                {
                    'status': False,
                    'message': 'failed to add vehicle. (required data: vehicle_number, seat_capacity, mileage, type)'
                }
            )
        vehicle = Vehicle(
            vehicle_number=jsn['vehicle_number'],
            seat_capacity=jsn['seat_capacity'],
            mileage=jsn['mileage'],
            vehicle_type=jsn['type']
        )
        vehicle.save()
...
def update_vehicle(request):
    try:
        ...
        vehicle_fields = ['vehicle_number', 'seat_capacity', 'mileage', 'vehicle_type']
        jsn = request.data
        ...
        vehicle.vehicle_number = jsn['vehicle_number'] if 'vehicle_number' in jsn else vehicle.vehicle_number
        vehicle.seat_capacity = jsn['seat_capacity'] if 'seat_capacity' in jsn else vehicle.seat_capacity
        vehicle.mileage = jsn['mileage'] if 'mileage' in jsn else vehicle.mileage
        vehicle.vehicle_type = jsn['type'] if 'type' in jsn else vehicle.vehicle_type
        vehicle.save()
        return Response(
            {
                'status': True,
